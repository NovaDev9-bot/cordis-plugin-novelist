...
## 2026-09-22（当晚六次）· B1：七份 agent 定义由生成器渲进宿主载体（v0.1.11）

**改了什么**：① 新增安装器 `scripts/install-wb-agents.mjs`（渲前言块＋封名单 → 宿主载体目录；`--check` 陈旧门；`--print`／`--dry-run`）；② 能力表新增 **`codebuddy.denyExempt`**（宿主豁免，逐条必须带理由）与每角色的 `agentdef`／`description`／`maxTurns`；③ 生成器多吐一件机器可读派生件 `references/carrier-deny.json`（**安装器只消费它，不许自己再算一遍**——再算一遍就是第二份实现；文件名走 ASCII：非文档件的文件名不许含中文，这条是棘轮判据，实测中文名当场把该项从 0 顶到 1）；④ **七份定义已装进宿主载体**（原来是 0 份：主笔那份是对方手装的，五个工种压根没有定义）；⑤ 三处被本轮实测推翻的旧话已改（主笔的「宿主适配层」、主编的降级表、五个工种派工文本的生成区）。

**三层分工（不许合并，写在这里免得后人省事合成一层）**：
1. **人格文本**＝包内 `agents/*.md`〔模板〕 / `references/roles/*.md`〔模板〕（真源 `roles/persona/`）——**不带封名单**。理由是硬的：官方校验器对前言块做**子串**判断，出现 `tools:` 即报错，而 `disallowedTools:` 含该子串 ⇒ 渲进包内 MD 立刻不合规。
2. **宿主载体**＝`<宿主配置目录>/agents/<role>.md`——前言块＋封名单，名字从能力表渲、**只渲 enforced**。
3. **陈旧门**＝`--check` 逐文件比内容——定义是**热加载**的，"装的时候对过"不作数。

**两处按本宿主重定的口径**（不是照搬 DSH）：
- **主笔的 `Glob`/`Grep` 有意不封**（对应能力表里的 `denyExempt: fs.search`，带理由）：本形态主笔要按纪律重读受影响正文、按身份取参照卡，只读面**不构成落账旁路**；DSH 形态封它是因为那边主笔没有路径知识、放开等于整块硬盘——**那条理由随宿主变**。**`Bash` 仍封**（它是一条能把账本文件直接改掉的旁路）。
- 五个工种的封名单＝15 个 MCP 名 ＋ `Read`/`Glob`/`Grep`/`Bash`/`ToolSearch`/`DeferExecuteTool`/`SendMessage`（按形态解析，实测全部 enforced）。**已实测拦不住的 `PowerShell` 不渲**——但它在安装器的输出与 `references/宿主工具面.md` 里**当缺口印出来**，不许静默。

**判据**：新增回归 `test/install-wb-agents.test.mjs` **9/9**（含四条负例：封名单混进实测拦不住的名字→拒渲／混进表外名字→拒渲／豁免缺理由→拒渲／正文源缺人格锚点→拒渲；另有陈旧门三态与"不碰别人文件"）；门禁新增第 15 步「宿主载体一致」（未装显式 skip）。
