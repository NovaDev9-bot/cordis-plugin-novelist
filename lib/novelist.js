 ...
 * hook=章末钩子（结构审计批 2026-09-18）/ checkpoint=抗压缩检查点。
 *
...
/**
 * 静默类（结构审计批 2026-09-18）：hook/payoff 每章各一条，不占窗口位、只计数。
 *
...
/** 卷尾结算的提醒宽限（章）：卷收束后仍报结算清单的章数（一批 6 章≈一个批次），过了就静默。 */
...
async function appendTape(fs, dir, args) {
  ...
  if (kind === 'hook' && args.closes != null) {
    ...
    throw new Error('hook 不接受 closes（钩子=章末短程拉力，无期限概念）——有明确兑现章的长线请用 kind=open_thread')
  }
  if (args.closes_thread) {
    const target = entries.find((e) => e.id === args.closes_thread && TAPE_CLOSABLE_KINDS.has(e.kind))
    if (!target) throw new Error('closes_thread 引用未知欠线/钩子 id：' + args.closes_thread + '（先 read 对账）')
    if (entries.some((e) => e.closes_thread === args.closes_thread)) throw new Error('欠线/钩子 ' + args.closes_thread + ' 已闭合过（append-only，勿重复闭线）')
...
/**
 * 幂等追加（结构审计批 2026-09-18）：同 kind + 同 ch + 逐字同 what 且未被作废的条目已在带 =
 * 这是重试/断点续跑，不重复入账。
 *
...
async function readTapeWindow(fs, dir) {
  ...
  // 未闭钩子（结构审计批）：钩子不进粘性窗口（每章一条会把安全阀顶爆），但"还欠哪些钩子"必须完整可查，
 ...
 */
function arcsFile(dir) { return dir + '/editorial/arcs.jsonl' }
...
const TOOLS = [
  ...
  // 2. novel_outline：写/读卷章纲
  {
    ...
    parameters: bookDirParam({
      properties: {
        ...
        // 章号参数名统一为 ch（与其余 13 个工具一致）；chapter_no 保留为**兼容别名**——
        // 它是章纲条目里的字段名（数据），不是入参该用的名字。统一批 2026-09-18：同义词只留一个正名，
        // 旧名继续接受（不破坏既有调用方），但新写的一律用 ch。
    ...
    async execute(args, exec) {
      ...
      if (!args.volume || !chNo || !args.entry) throw new Error('write 需要 volume/ch/entry')
  ...
  // 4. novel_chapter：正文落盘 + 自动记账 + 内联符号门（唯一正稿写入口）
  {
    ...
    parameters: bookDirParam({
      properties: {
        ...
        hook: { type: 'object', additionalProperties: true, description: '可选（v7.9 派生）：本章章末钩子，落事件带 kind=hook { what ≤60字=钩子本身, why ≤60字=它靠什么拽住读者 }。每章一条是读者契约第⑤条（章末必须有下章钩子）；未兑现的钩子进 open_hooks，超 ' + HOOK_STALE_CHAPTERS + ' 章未见兑现 verify 报一条。有明确兑现章的长线用 seeds（伏笔，有 due_ch 逾期对账），不要用 hook。' },
        pays_hooks: { type: 'array', items: { type: 'string' }, description: '可选（v7.9 派生）：本章兑现掉的钩子 id 列表（事件带 tNNN）——闭线走与欠线同一套对账（closes_thread），重复闭线拦。' },
    ...
    async execute(args, exec) {
      ...
      // 钩子预检（v7.9，同 advance_to 的道理）：事件带是本章账务的最后一步，参数错（未知钩子 id /
      ...
      // 章末钩子（v7.9）：钩子与欠线同一条因果链，共用一个账本一套闭线对账。
  ...
  // 5. novel_verify：符号级自检（只读）
  {
    ...
    async execute(args, exec) {
      ...
      // （钩子超期、卷尾结算）。分开的理由（结构审计批 2026-09-18）：卷尾结算每次卷末必然出一条，
      ...
      // 有效投影（v7.9 与 v7.5 对齐）：被 supersedes 作废的条目不参与逾期/未闭判定——
      ...
      if (tapeLines.length) {
        ...
        // 章末钩子（v7.9）：两查——①超期未兑现（warn 级）②近窗漏记。
      ...
      // 卷尾结算（v7.9）：卷收束后仍报一个批次（VOLUME_SETTLE_GRACE 章）的结算清单，之后静默。
      ...
      for (const vol of outline.volumes || []) {
        ...
        warnings.push('卷 ' + vol.volume + ' 结算（末章 ' + last + '，已过 ' + (cur - last) + ' 章）：' + parts.join('；') + '——跨卷延续是允许的，此处只作结算记录')
      }
      // 弧审账单核对（v7.9，ADR-034 弧级审查纪律）：卷收束且过了宽限期仍无结构化弧审条目 → 报一条。
  ...
  // 6. novel_ledger：台账定向增改 + 冲突仲裁
  {
    ...
    parameters: bookDirParam({
      properties: {
        op: { type: 'string', enum: ['update_character', 'update_term', 'update_foreshadow', 'resolve_conflict', 'close_foreshadow', 'set_chapter_status', 'arc_review'], description: '操作类型' },
        arc: {
    ...
    async execute(args, exec) {
      ...
      if (args.op === 'arc_review') {
        // 弧审账单（v7.9）：弧级审查的**结构化产出**。报告（reports/弧审-卷X-弧Y.md）仍然是给人读的，
  ...
  // 6.8 novel_ask：问账本原语（总蓝图 R1 · v7.0）——语义查询入口，代码替主编跑"查哪几个文件+拼过滤条件"
  {
    ...
    async execute(args, exec) {
      ...
      if (wantsTape || matches.length === 0) {
        ...
        // 未闭钩子（v7.9）：钩子不进窗口（每章一条会把画框占满），所以问"还欠什么"时单独带出来——
        // 账上记了却问不出来，等于没记。
  ...
  },
  {
    ...
    output: {
      // 封闭 schema（契约统一批 2026-09-18）：此前是 additionalProperties:true，调用方无法从契约
      // 知道会拿到什么。三 op 的返回键取并集——宁可宽一点，也要让"有哪些键"是**可枚举**的。
      schema: {
        type: 'object',
        properties: {
    ...
    async execute(args, exec) {
      ...
      if (op === 'factsheet') {
        // 确定性六节投影（A 批缩形版）：只搬账本里**已经存在**的事实，不判断、不概括、不代写。
        ...
        const sections = {
          ...
          open_hooks: [
            ...
            // 未闭钩子（v7.9）：上一章埋的钩子正是本章要接住的东西——事实卡缺了它，主笔就是在盲续。
          ...
          timeline_anchors: (tml.events || []).filter((e) => e.ch == null || e.ch <= ch).slice(-5)
  ...
  },
  {
    ...
    output: {
      // 契约封闭化（统一批 2026-09-18）：命中是候选不是事实，故同时返回扫描面与截断标志，
      // 让调用方能自己判断"这条召回值不值得信"。
      schema: {
        type: 'object',
        properties: {
    ...
    async execute(args, exec) {
      ...
      let names = []
      try { names = (await fs.listDir(await fs.resolve(dir + '/manuscript'))).map((e) => e.name) } catch (e) { /* 尚无正文 */ }
      ...
      for (const { name, ch } of chapters) {
        ...
        scanned += 1
        const text = String(await fs.readText(await fs.resolve(dir + '/manuscript/' + name))).replace(/\r\n?/g, '\n')
