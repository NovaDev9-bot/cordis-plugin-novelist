    ...
    async execute(args, exec) {
      ...
      if (args.advance_to) {
        ...
        if (preFrom !== to && !(CHAPTER_TRANSITIONS[preFrom] || []).includes(to)) {
          throw new Error('advance_to 非法状态迁移：' + preFrom + ' → ' + to + '（合法去向：' + ((CHAPTER_TRANSITIONS[preFrom] || []).join('/') || '无') + '；正文尚未落盘，可改参重交或改用 ledger set_chapter_status）')
        }
      }
...
const _internals = { countHan, symbolGate, chapterFile, chapterTitle, CHAPTER_STATES, CHAPTER_TRANSITIONS, TOOLS, SECTION, loadJson, appendJsonlLine, readJsonlLines, appendTape, readTapeWindow, TAPE_KINDS, sanitizeOwn }
