...
const TOOLS = [
  ...
  // 5. novel_verify：符号级自检（只读）
  {
    ...
    async execute(args, exec) {
      ...
      const missingPlanned = [...inOutlineNos].filter((n) => !chapters.includes(n)).sort((a, b) => a - b)
      const missingUnplanned = []
      for (let n = 1; n <= cur; n++) if (!chapters.includes(n) && !inOutlineNos.has(n)) missingUnplanned.push(n)
      const CAP = 6
  ...
  // 服务端**确实**把 11k 字的 guide 放在 initialize 的 instructions 字段里（实测长度 11436），
  ...
  // 6.7 novel_decide：裁决原语（总蓝图 R1/R5）——"说一句话，账自动动"
  {
    ...
    async execute(args, exec) {
      ...
      // 派生③：撤销语义——supersedes 引用既有条目即作废（append-only 不改历史）
      if (args.supersedes) {
        const entries = await readTapeEntries(fs, dir)
        const target = entries.find((e) => e.id === String(args.supersedes))
        if (!target) throw fail(ERR.NOT_FOUND, 'supersedes 引用未知事件带条目：' + args.supersedes, '先 novel_event op=read 对账')
