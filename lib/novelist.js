...
const CRAFT_CARDS_PATH = (() => {
  try {
    ...
    return existsSync(p) ? p : 'craft/author-cards/（本仓库内，随插件分发）'
  } catch (e) { return 'craft/author-cards/（本仓库内，随插件分发）' }
})()
...
async function appendTape(fs, dir, args) {
  ...
  if (!args.actor) throw fail(ERR.BAD_ARG, 'actor 必填（谁记的：主编/策划/档案员…）')
...
const TOOLS = [
  ...
  // 5. novel_verify：符号级自检（只读）
  {
    ...
    async execute(args, exec) {
      ...
      // 实测（2026-09-18）：fangxiangyi-01 的章纲全无 choice_axis（载荷早于 ADR-034），verify 输出 0 issues。
      //
