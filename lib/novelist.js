...
const CRAFT_CARDS_PATH = (() => {
  try {
    ...
    return existsSync(p) ? p : 'craft/author-cards/（本插件安装目录下没有 craft/ —— npm 装法不含作家卡；从仓库安装才有，或向主编要卡内容）'
  } catch (e) { return 'craft/author-cards/（本插件安装目录下没有 craft/ —— npm 装法不含作家卡；从仓库安装才有，或向主编要卡内容）' }
})()
...
const SECTION = {
  ...
  text: [
    ...
    '商业工位（M2/M3）：novel_outline 定稿后策划交书名候选×5+简介×2（协议=editorial/protocols/书名简介工位.md；主编合并后呈 Owner 三选一/二选一，书名=点开率生死线不自主决定）；存稿≥10 章起主编按 editorial/protocols/责编评分卡-番茄.md 自评（**六维**带证据坐标，**<20 分不送外审，≥24 分才呈送**——口径以卡片 v2.2 为准，卡片自己注明"五维/<17/≥20"是已作废旧版）。',
