      ...
      if (op === 'factsheet') {
        ...
        const sections = {
          ...
          near_characters: (cj.characters || []).filter((c) => Number.isInteger(c.last_seen_ch) && c.last_seen_ch < ch && c.last_seen_ch >= ch - near)
            ...
            .map((c) => ({ name: c.name, identity: c.identity ?? null, last_seen_ch: c.last_seen_ch })),
          world_rules: (bible.terms || []).filter((t) => t.effective_from_ch == null || t.effective_from_ch <= ch)
            .map((t) => ({ name: t.name, value: t.value, from_ch: t.effective_from_ch ?? null })),
...
const SECTION = {
  ...
  order: 150,
  text: [
    'novelist-guide v7.8 · 机制细则唯一真源（novelist 插件同版注入；preset persona 只含决策原则与指针）。书工程 = book_dir（一部书一个目录；novel_* 工具一律显式传 book_dir 绝对路径）。身份与协作框架见 persona 段。**机制沿革（v6→v7 逐版增量：新增机制/口径修正/作废条款）见 ' + GUIDE_HISTORY_PATH + '——只在对某版细节有疑问时定向读，不必常驻上下文；本条以下各段即为当前生效口径。**',
    '检索与事实卡（v7.8，2026-09-18 A+B 批）：①**正文检索 novel_search**——在 manuscript/ 当前正文里做字面查找，返回 章号＋行号＋命中片段；多词空格分隔=AND；ch/window 限定章范围；触碰 limit 明报截断。**它是候选召回不是事实源**（引用/改稿前读原章核对；账本才是真值），不做语义近似、不做摘要、不读版本快照。**主笔可直调**（自查旧文的唯一通道：宿主权限按工具名不按路径，主笔被 deny glob/grep＝连书目录都搜不了；放开=放开整块硬盘）。盲角色（试读员/校准员/校对员/拆书员）连同 novel_context 一并 deny。②**前情事实卡投影 novel_context op=factsheet**——按章投影六节确定性事实：应兑现伏笔（due ≤ ch，逐条标逾期）/既有钩子（未来到期伏笔＋未闭欠线）/近场人物（近 near 章出现过，缺省 8）/世界观规则（effective ≤ ch）/时间线锚（≤ ch 最近 5 条）/禁改清单（近 20 章已收伏笔）；ch 缺省=current_ch+1，每条带章号坐标。**叙述性一节（上章末状态一段话）不代写**——返回里明标由主编读上一章正文后补写。近场人物≠本章实际出场者：主编按细纲增删。',
