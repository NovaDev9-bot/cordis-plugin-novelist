...
import {
  ...
  LOCK_DEFAULTS, enqueueBook, acquireFileLock, classifyLockError, withWriteGate,
...
async function appendJsonlLine(fs, file, line) {
  ...
  try {
    ...
    // 降级读改写（writeText 后端 temp+rename 原子）。**行尾纪律（压测暴露的 bug 修复
...
const TOOLS = [
  ...
  // 3. novel_bible：写前必查（防吃书）
  {
    ...
    parameters: bookDirParam({
      properties: {
        ...
        scope: { type: 'string', default: 'all', enum: ['all', 'term', 'character', 'foreshadow', 'timeline', 'rule'], description: '查询范围（缺省 all）' },
        name: { type: 'string', description: '可选：精确查找某个术语/人物' },
      },
      required: ['book_dir', 'ch'],
    }),
    output: {
      schema: { type: 'object', properties: { ch: { type: 'integer' }, terms: { type: 'array' }, characters: { type: 'array' }, foreshadows: { type: 'array' }, timeline: { type: 'array' }, rules: { type: 'array' } }, additionalProperties: false },
    ...
    async execute(args, exec) {
      ...
      const rules = await loadJson(fs, dir, 'rules.json') || { rules: [] }
      return {
        ch,
        ...
        timeline: (scope === 'all' || scope === 'timeline')
          ...
          : [],
        // 规矩同样按生效窗投影（2026-09-22）：给的是"写到第 ch 章为止这条规矩长什么样"，
        // 包括它被改过几笔——写前查规矩与查设定是同一件事，不该靠作者记忆。
        rules: (scope === 'all' || scope === 'rule') ? pick(args.name, rules.rules, eff) : [],
      }
    },
  },
  ...
  // 5. novel_verify：符号级自检（只读）
  {
    ...
    async execute(args, exec) {
      ...
      // 读感记录缺来源凭证（warning；盲读协议 v1.3 / 整改总计划 P2-C1，2026-09-21）
      // 协议要求"切包即产输入清单（blind-manifest.mjs），交付件首行回填 root_hash"——
      // 在**没有工具级 deny 的宿主形态**下，这是唯一能机器取证的环节（读了包外的东西，哈希对不上）。
      // 缺凭证**不是格式问题**：它的意思是"这份读感依据的究竟是哪些字节"不可考，证据力降一档；
      // 所以文案点名写死，别让读者把它当排版疏漏。
      // 分级=warning 不判 issue：存量书（口径变更前派工的）会成片缺凭证，全打红等于把红灯贬值。
      // 两处落点都查：批审采样落 editorial/blind-samples/<批id>/，单发试读落 orders/block-XXX/试读员-意见.md。
      {
        const cand = []
        const pushDir = async (d, filter) => {
          let entries = []
          try { entries = (await fs.listDir(await fs.resolve(dir + '/' + d))) || [] } catch (e) { return }   // 目录不存在＝这一类还没用，不报
          for (const e of entries) {
            if (filter && !filter(e.name)) continue
            const rel = d + '/' + e.name
            const st = await fs.stat(await fs.resolve(dir + '/' + rel)).catch(() => null)
            if (!st) continue
            // 两种适配形状都认：DSH fs 服务与 MCP 适配器给的是**布尔字段** `isDirectory`，
            // node:fs 的 Stats 给的是**方法**。只认一种，另一种会把所有目录当文件——
            // 而失败方式是静默的（readText 读目录抛错被 catch 掉，于是"一条记录都没找到"）。
            const isDir = typeof st.isDirectory === 'function' ? st.isDirectory() : !!st.isDirectory
            if (!isDir) { cand.push(rel); continue }
            let inner = []
            try { inner = (await fs.listDir(await fs.resolve(dir + '/' + rel))) || [] } catch (err) { continue }
            for (const f of inner) cand.push(rel + '/' + f.name)
          }
        }
        await pushDir('editorial/blind-samples', null)
        await pushDir('orders', null)
        const recs = cand.filter((r) => /\.(md|json)$/i.test(r) &&
          (/blind-samples\//.test(r) || /试读员/.test(r)))
        let missing = []
        for (const r of recs) {
          let t = ''
          try { t = await fs.readText(await fs.resolve(dir + '/' + r)) } catch (e) { continue }
          if (!t.includes('root_hash')) missing.push(r)
        }
        if (recs.length && missing.length) {
          warnings.push('读感记录缺来源凭证（root_hash）：' + missing.length + '/' + recs.length + ' 份 —— ' +
            missing.slice(0, 6).join('、') + (missing.length > 6 ? ' 等' : '') +
            '。**本记录缺 root_hash，来源不可考，证据力降级**（不是格式问题）。' +
            '补法：主编按盲读协议 §一 用 blind-manifest.mjs 生成输入清单并回填；' +
            '代填（非 DSH 形态下唯一可行）必须在记录里注明"回填主体＝主编"——那只能证到"输入一致"，不证明输入唯一')
        }
      }
      ...
      // ── 规矩一致性（2026-09-22 · novel_ledger op=rule）──────────────────────────
      // **采用门**：书里一条规矩都没声明 ⇒ 本项不启用（存量书不误报，与 anchor_params 同款）。
      // 启用后查两件：①同一生效窗里同名规矩有两套正文（正常路径被 op=rule 拦住，这里兜手改账本
      // 与外部导入）；②章纲声明的 rules_used 必须存在、且在第 chapter_no 章生效。
      const rulesDoc = await loadJson(fs, dir, 'rules.json')
      const allRules = (rulesDoc && Array.isArray(rulesDoc.rules)) ? rulesDoc.rules : []
      let rulesDeclaredInOutline = false
      for (const vol of outline.volumes || []) {
        for (const c of vol.chapters || []) if (Array.isArray(c.rules_used) && c.rules_used.length) rulesDeclaredInOutline = true
      }
      if (allRules.length || rulesDeclaredInOutline) {
        const winText = (x) => (x.effective_from_ch == null ? '全书起' : '第 ' + x.effective_from_ch + ' 章起') + '～' + (x.effective_to_ch == null ? '全书' : '第 ' + x.effective_to_ch + ' 章止')
        const byName = new Map()
        for (const r of allRules) {
          for (const o of byName.get(r.name) || []) {
            if (windowsOverlap(o, r) && o.text && r.text && o.text !== r.text) {
              issues.push('规矩「' + r.name + '」在同一生效窗里有两套正文（' + winText(r) + '：「' + String(o.text).slice(0, 24)
                + '…」 vs 「' + String(r.text).slice(0, 24) + '…」）——走 novel_ledger op=resolve_conflict 仲裁，'
                + '或给新正文一个不重叠的 effective_from_ch（"改一笔"要留版本痕迹，别覆盖历史）')
            }
          }
          byName.set(r.name, (byName.get(r.name) || []).concat([r]))
        }
        for (const vol of outline.volumes || []) {
          for (const c of vol.chapters || []) {
            for (const nm of (Array.isArray(c.rules_used) ? c.rules_used : [])) {
              const hits = allRules.filter((x) => x.name === nm)
              if (!hits.length) {
                issues.push('第 ' + c.chapter_no + ' 章章纲声明用到规矩「' + nm + '」，但账本里没有这条规矩——先 novel_ledger op=rule 落账'
                  + '（规矩不上账＝后面没人能核"这条规矩有没有被破"）')
                continue
              }
              const eff = hits.filter((x) => (x.effective_from_ch == null || x.effective_from_ch <= c.chapter_no)
                && (x.effective_to_ch == null || c.chapter_no <= x.effective_to_ch))
              if (!eff.length) {
                issues.push('第 ' + c.chapter_no + ' 章章纲声明用到规矩「' + nm + '」，但该章不在任何版本的生效窗内'
                  + '（既有窗：' + hits.map(winText).join(' / ') + '）——要么改章纲，要么给它一个覆盖本章的版本')
              }
            }
          }
        }
  ...
  // 6. novel_ledger：台账定向增改 + 冲突仲裁
  {
    ...
    parameters: bookDirParam({
      properties: {
        op: { type: 'string', enum: ['update_character', 'update_term', 'update_foreshadow', 'resolve_conflict', 'close_foreshadow', 'set_chapter_status', 'arc_review', 'seal_book', 'rule'], description: '操作类型' },
        ...
        rule: { type: 'object', additionalProperties: true, description: 'op=rule 时（2026-09-22）：{ name 规矩名, text 一句话正文（必填）, source? 来处（祖上传的/某年立的/谁改的）, effective_from_ch?, effective_to_ch?, evidence_ch?, note? }。**规矩是这本书的世界法则**：与词条同一套生效窗语义（窗口不重叠＝演进，追加一条且旧值留档；窗口重叠且正文不同＝真冲突，走 resolve_conflict）——"这条规矩哪年生效、被谁改过"因此是可查的账，而不是作者的记忆。启用门：书里一条规矩都没声明＝本项不启用（存量书不误报）。' },
    ...
    async execute(args, exec) {
      ...
      if (args.op === 'resolve_conflict') {
        ...
        if (c.verdict === 'accept_new') {
          // 〔2026-09-20 第三方复核修复〕按 **target 生效窗**定位被冲突的那条记录：
          // v7.9 起同名可有多条版本记录，旧实现 `.find(name)` 只命中首条——裁决会写错那条
          // 且不报。legacy 冲突事件（早于本修复、无 target 字段）：同名只有一条时按旧行为
          // 写它；同名多条时**拒写并说明**（宁可不写，不静默写错——共享账的错写会污染全书口径）。
          const winOf = (x) => [(x.effective_from_ch ?? null), (x.effective_to_ch ?? null)]
          const sameWin = (x, t) => JSON.stringify(winOf(x)) === JSON.stringify([t.effective_from_ch ?? null, t.effective_to_ch ?? null])
          const pickTarget = (list) => {
            if (conf.target) {
              const hit = list.find((x) => x.name === conf.name && sameWin(x, conf.target))
              if (!hit) throw fail(ERR.NOT_FOUND, '裁决定位失败：账上找不到生效窗=' + JSON.stringify(conf.target) + ' 的同名记录（' + conf.name + '）',
                '该记录可能已被后续写入改写窗口；用 novel_bible 读出当前同名各条的窗口，再决定用 update_character/update_term 直接改目标那条')
              return hit
            }
            const named = list.filter((x) => x.name === conf.name)
            if (named.length <= 1) return named[0] || null
            throw fail(ERR.CONFLICT, '这条冲突事件（' + conf.id + '）早于 target 坐标登记，而同名记录有 ' + named.length + ' 条——无法判定该写哪条，拒写',
              '用 novel_bible 读出各条窗口，然后 update_character/update_term 直接改目标窗口那条（legacy 事件不做猜测性回写）')
          }
          if (conf.kind === 'character_gender') {
            const cj2 = await loadJson(fs, dir, 'characters.json') || { characters: [] }
            const ch2 = pickTarget(cj2.characters)
            ...
            const t2 = pickTarget(bible2.terms)
            if (t2) { t2.value = conf.new; await saveJson(fs, dir, 'bible.json', bible2) }
          } else if (conf.kind === 'rule_text') {
            // 2026-09-22 · op=rule：规矩正文冲突的胜诉回写（上面那条维护警示指的就是这一处——
            // 新增冲突产生点必须同步回写分支，否则「裁决胜诉」在账上落一条，规矩正文却一个字没变）
            const rules2 = await loadJson(fs, dir, 'rules.json') || { rules: [] }
            const r2 = pickTarget(rules2.rules)
            if (r2) { r2.text = conf.new; await saveJson(fs, dir, 'rules.json', rules2) }
          }
        }
      ...
      if (args.op === 'update_character') {
        ...
        if (clash) {
          ...
          // target 坐标＝被冲突的那条**具体版本记录**的生效窗。v7.9 起同名可有多条记录
          // （窗口不重叠＝口径演进），只记 name 的话裁决回写会命中"首条同名"——写错那条
          // 且不报（2026-09-20 第三方复核指出的静默错写路径）。
          const conflictTarget = { effective_from_ch: clash.effective_from_ch ?? null, effective_to_ch: clash.effective_to_ch ?? null }
          await appendEvent(fs, dir, { op: 'conflict', id: conflictId, scope: 'semantic', kind: 'character_gender', name: c.name, old: clash.gender, new: c.gender, target: conflictTarget })
          return { ok: false, result: { conflict: { id: conflictId, kind: 'character_gender', name: c.name, old: clash.gender, new: c.gender, target: conflictTarget, message: '性别与既有人物卡在同一生效窗内冲突，请 resolve_conflict 仲裁' } } }
        }
      ...
      if (args.op === 'update_term') {
        ...
        if (clash) {
          ...
          const conflictTarget = { effective_from_ch: clash.effective_from_ch ?? null, effective_to_ch: clash.effective_to_ch ?? null }
          await appendEvent(fs, dir, { op: 'conflict', id: conflictId, scope: 'semantic', kind: 'term_value', name: t.name, old: clash.value, new: t.value, target: conflictTarget })
          return { ok: false, result: { conflict: { id: conflictId, kind: 'term_value', name: t.name, old: clash.value, new: t.value, target: conflictTarget, message: '词条值在同一生效窗内冲突，请 resolve_conflict 仲裁（若这是口径演进，请给新值一个不重叠的 effective_from_ch）' } } }
        }
      ...
      // op=rule（2026-09-22）：规矩＝这本书的世界法则，与词条同一套生效窗语义。
      // 为什么单独立账而不是塞进 bible.terms：规矩会被"逐条拆"（读者、试读员、校准员都会），
      // 而"这条规矩哪年生效、被谁改过"必须可查——这正是候选 A「规矩的修订者」的金手指本身。
      if (args.op === 'rule') {
        const store = await loadJson(fs, dir, 'rules.json') || { rules: [] }
        if (!Array.isArray(store.rules)) throw fail(ERR.BAD_ARG, 'rules.json 结构不对：期望 { rules: [] }')
        const r = args.rule
        if (!r || !r.name) throw fail(ERR.BAD_ARG, 'op=rule 需要 rule.name（规矩名，如「闸规」「停灵规」）')
        if (!r.text || !String(r.text).trim()) {
          throw fail(ERR.BAD_ARG, 'op=rule 需要 rule.text：一句话把这条规矩写清楚——读者会逐条拆规矩，写不清楚就是给自己埋雷')
        }
        const sameName = store.rules.filter((x) => x.name === r.name)
        const clash = sameName.find((x) => windowsOverlap(x, r) && x.text && r.text && x.text !== r.text)
        if (clash) {
          const conflictId = 'r_' + Date.now()
          const conflictTarget = { effective_from_ch: clash.effective_from_ch ?? null, effective_to_ch: clash.effective_to_ch ?? null }
          await appendEvent(fs, dir, { op: 'conflict', id: conflictId, scope: 'semantic', kind: 'rule_text', name: r.name, old: clash.text, new: r.text, target: conflictTarget })
          return {
            ok: false,
            result: {
              conflict: {
                id: conflictId, kind: 'rule_text', name: r.name, old: clash.text, new: r.text, target: conflictTarget,
                message: '规矩正文在同一生效窗内冲突，请 resolve_conflict 仲裁（若这是「规矩被改了一笔」，请给新正文一个不重叠的 effective_from_ch——那才是这本书的机制，不是覆盖历史）',
              },
            },
          }
        }
        const target = sameName.find((x) => windowsOverlap(x, r))
        if (target) Object.assign(target, sanitizeOwn(r)); else store.rules.push(sanitizeOwn(r))
        await saveJson(fs, dir, 'rules.json', store)
        await appendEvent(fs, dir, { op: 'rule_update', name: r.name })
        return { ok: true, result: { saved: r.name, versioned: !target, versions: store.rules.filter((x) => x.name === r.name).length, total: store.rules.length } }
      }
  ...
  {
    name: 'novel_context',
    description: '账本分层回看（只读、零 LLM）：op=summary 按 chapter/volume/book 投影章纲标题、出场 cast、埋收伏笔 id、时间线条数与章纲字数区间；未知字段为 null，不推断正文。chapter 缺省只给最近 24 章明细，更早仅计数，window={from,to} 闭区间可显式扩展。volume/book 只给聚合计数，不代替读正文。op=checkpoint 返回事件带最近检查点与章号，无则明报尚无 checkpoint。op=factsheet 按章投影"前情事实卡"的**确定性七节**（应兑现伏笔/既有钩子（未来到期伏笔＋未闭欠线＋**未闭章末钩子 tNNN**）/近场人物/世界观规则/时间线锚/禁改清单/**规矩**（按生效窗投影，2026-09-22），每条带章号坐标；ch 缺省=current_ch+1，near=近场人物窗口默认 8 章）；叙述性一节（上章末状态一段话）**不代写**，由主编补。读取账本与编辑部意图，盲角色禁用。',
    ...
    async execute(args, exec) {
      ...
      if (op === 'factsheet') {
        // 确定性七节投影：只搬账本里**已经存在**的事实，不判断、不概括、不代写。
        ...
        const bible = await loadJson(fs, dir, 'bible.json') || { terms: [] }
        const tml = await loadJson(fs, dir, 'timeline.json') || { events: [] }
        const rulesStore = await loadJson(fs, dir, 'rules.json') || { rules: [] }
        const tape = await readTapeWindow(fs, dir)
        ...
        const sections = {
          ...
          // 规矩（2026-09-22）：按生效窗投影到本章——给的是"写到第 ch 章为止这条规矩长什么样"，
          // 连它被改过几笔一起给（同一名字多条版本＝这本书的机制本身）。
          rules: (rulesStore.rules || []).filter((r) => windowCovers(r, ch))
            .map((r) => ({ name: r.name, text: r.text, source: r.source ?? null, from_ch: r.effective_from_ch ?? null, to_ch: r.effective_to_ch ?? null, versions: (rulesStore.rules || []).filter((x) => x.name === r.name).length })),
        }
        return {
          op, ch, near, sections,
...
const SECTION = {
  ...
  text: [
    ...
    '生效窗与人物欲求（v7.9，2026-09-18 结构审计批）：①**词条/人物卡是带生效窗的版本记录**——同一名字可有多条记录，按 ch 投影取"窗口包含本章"的那条。窗口不重叠的新值＝口径演进（追加一条，旧值与旧窗口原样留档，**不回头篡改历史**）；**窗口重叠且值不同才是真冲突**（走 resolve_conflict 仲裁）。缺窗口字段＝全书窗（旧账本行为不变）。写作纪律：设定/人设演进一律给新值一个从新章起算的 effective_from_ch（必要时给旧值补 effective_to_ch），别用同名覆盖去"更正历史"。②**人物欲望必须进账**——characters.json 记 want（贯穿欲求，全书级一句话）与 opposed_by（主要压力来源）；主笔的"人物欲望设计"若不落账，换会话就重建不出来（账本是唯一可重建源），而长篇最典型的死法正是"主角连续多章没有想要的东西"。③结构校准员的病类目录＝**九类**（A1-A4 形成过程被结论替代 ＋ B5-B9 叙事组织过满；旧称"八类"是 v6.6 加回 B9 后未同步的名字，已统一为九类）。④**规矩进账本（v7.16，2026-09-22）**：`novel_ledger op=rule` 声明这本书的世界法则 `{name, text, source?, effective_from_ch?, effective_to_ch?}`，与词条同一套生效窗语义——**窗口不重叠的新正文＝"这条规矩被改了一笔"（追加一条、旧版本原样留档），窗口重叠且正文不同才是真冲突**（走 resolve_conflict）。写作纪律：改规矩一律给新正文一个从新章起算的 `effective_from_ch`，别用同名覆盖去"更正历史"——**这本书的机制就是"规矩有版本痕迹"，覆盖掉它等于把机制本身删了**。查：`novel_bible scope=rule`（按章投影）／`novel_context op=factsheet` 的「规矩」节（派工包账本事实自带）。核：章纲里写 `rules_used: [规矩名…]`，`novel_verify` 查它**存在且在该章生效**（引用不存在的规矩、或引用了该章不生效的版本都报 issue）；**采用门＝书里一条规矩都没声明就不启用**（存量书不误报）。',
    '检索与事实卡（v7.8，2026-09-18 A+B 批）：①**正文检索 novel_search**——在 manuscript/ 当前正文里做字面查找，返回 章号＋行号＋命中片段；多词空格分隔=AND；ch/window 限定章范围；触碰 limit 明报截断。**它是候选召回不是事实源**（引用/改稿前读原章核对；账本才是真值），不做语义近似、不做摘要、不读版本快照。**主笔可直调**（自查旧文的唯一通道：宿主权限按工具名不按路径，主笔被 deny glob/grep＝连书目录都搜不了；放开=放开整块硬盘）。盲角色（试读员/校准员/校对员/拆书员）连同 novel_context 一并 deny。②**前情事实卡投影 novel_context op=factsheet**——按章投影七节确定性事实：应兑现伏笔（due ≤ ch，逐条标逾期）/既有钩子（未来到期伏笔＋未闭欠线＋**未闭章末钩子 tNNN**，见 v7.10）/近场人物（近 near 章出现过，缺省 8）/世界观规则（effective ≤ ch）/时间线锚（≤ ch 最近 5 条）/禁改清单（近 20 章已收伏笔）/**规矩**（世界法则，按生效窗投影到 ch；同名多条版本＝这本书的机制，见 v7.16）；ch 缺省=current_ch+1，每条带章号坐标。**叙述性一节（上章末状态一段话）不代写**——返回里明标由主编读上一章正文后补写。近场人物≠本章实际出场者：主编按细纲增删。',
    ...
    '工作流（每章，v7.0 三原语执行流——主编手工工具调用目标 ≤3 次/章〔v7.15 补边界〕；该指标**不含「一章收束验收」与批末聚合**——验收另有 5 步，那 5 步的工具动作不计入本指标）：动笔前细纲三行呈 Owner——①本章拍点（一句话节拍）②章末钩子③个体参照；关键章（卷首/卷尾/大转折/伏笔兑现/钩子切换）加展开细纲 Owner 全看，常规章三行点头、每 3 章抽 1 章展开，抽看见平/漂移则连续 3 章升格；章长窗默认 3000-4000 汉字，日节奏默认 6 章/批。执行：①novel_ask 一次查账（实体卡+伏笔欠线+时间线+事件带窗口由它一次拼好，替代 bible 手填过滤+event read+verify 翻账；查"写到某章为止"就带 ch——它按 ≤ch 投影，存稿不算当前账；要确定取数用 select）→ ②主编组装派工包派主笔（**结构＝固定层＋检索三段**〔v7.14 派工包结构化〕。固定层＝前情事实卡/前一章正文逐字（禁摘要替代）/上章反馈卡（Owner 裁决摘要+审稿分歧点——生成端的反馈回路，不许只给绿灯）/章纲末位/零编排对白混入/**个体参照卡**——把本卷走向呈报里定下的那张作家卡**内容全文贴进包**，取卡处＝' + CRAFT_CARDS_PATH + '。为什么必须贴全文：主笔的 glob/grep 被 deny（宿主权限按工具名不按路径），它只能按精确路径 read，只给目录名等于没给；个体参照留在库里＝这一格空转）。**检索三段各带硬上限**（v7.14）：原文锚点＝与本章直接相关的 3–5 条前文片段（章号＋行号＋片段，每条 ≤200 汉字、合计 ≤1200）／未闭义务＝只列本章要接的未闭欠线与未兑现钩子（≤10 条）／账本事实＝`novel_context op=factsheet` 七节原样投影（实测 1–2k 汉字、不随书增长）。**"相关的都塞"是明确禁止项**——RULER 实测干扰针越多越差（极端掉约 40 分）、recall-bench 实测 1–2k 汉字即满分召回，多给不增召回只增干扰；三段数值为首版标定值，跑真书后用数据调）。**派工包里的数字纪律**〔v7.17〕：包内**不许出现文本表层统计**——对话占比／句长中位／短句占比这类分布量，以及"描写占比 X–Y%"这类篇幅配比，一律**不进写手派工包**（它们归审校侧 style-check / structure-check，标注「不进写手派工包」）。允许进包的只有两种形状：**结构节拍**（何事何时发生：300 字内进主题／每 500 字一钩／章末留卡点）与**形态上限**（"不许超过"，不是"争取达到"：句 ≤20 字／段 ≤5 行）。理由＝extremal Goodhart，**已实测翻车**：包内写「对话占比 ≥35%」→ 交回 45.8%、段均 1.4 行——指标对了、语域错了。另：**段的行数上限是 5（工作目标 3），不是 3**——"≤3 行"出自二手经验贴、比番茄官方（3–5 行）更严，且会被执行成"通篇一句一段"＝节奏被抹平。参照卡的形状由装配期守卫机器把关，**任务书是运行时拼的、机器管不到，靠这条纪律**。→ 主笔交细纲三行提案（**危险章先证据后落笔**〔v7.14〕：危险章＝卷内中段／多线交汇章／上一章机检旗标章，此类章的派工包加一行指令——动笔前先输出「本章依据清单」（引用哪些账本条目与锚点，**带章号坐标**）；主编收稿对照，清单外的设定出现＝无来源新设定，机检可对账）→ 呈 Owner 点头 → 主笔按裁定写正文（**两段式派工**：提案与正文是两个阶段，续接方式随宿主——DSH 端同会话续写，非 DSH 端另派一次且裁定原文进包）→ ③novel_chapter 一次收束（参数携带正文+seeds/closes/cast/timeline+decision 随章落带+**hook 落本章章末钩子／pays_hooks 兑现旧钩子**+advance_to 状态推进；完成回执落 editorial/txn/，断点续跑以回执 status=done 为准——残留 pending=半提交，同文本重交幂等补齐）→ ④批审（v7.4 起：主编按批审协议并行 spawn 盲采样员 N=2-3（近全票才加采至 6）→意见落 editorial/blind-samples/ → batch-aggregate 聚合 → 只看分歧点；过渡期单发试读/校准仍可用，批审为默认）→ ⑤意见回流入 novel_decide（裁决+状态迁移+落带一次完成；撤销=同工具 supersedes 引用旧条目）。**批末金丝雀**：每批随机抽 1 章走 A-B 盲读冷读（平信号哨兵，生产环路唯一能报"平"的通道，不得归零；细纲呈报趋稳后随 R4 降档条款上调频率）。novel_bible/novel_event/novel_ledger set_chapter_status 保留为细粒度兜底，常规章不再单独调用；novel_verify 批末跑一次全量（含新章符号门复核）。',
    ...
    '派工分工（v7.2 两座位制+按需工种+仪器）：**常设两座位**——主编=编排/裁决/落账权/ask 门；**主笔=作者+执笔一体**（原策划+写手合并：卷级 3 互斥走向血肉署名提案、章级细纲三行提案、人物欲望设计；两段式派工=提案→Owner 门→正文（**续接方式随宿主**：DSH 端同会话续写；非 DSH 端另派一次并把裁定原文进包）；派工线＝**可续派工线**（continuable，DSH 端可用——它是**运输方式不是地位**）；deny=不碰落账/不碰事件带/不碰 shell）。**按需工种**（一次性 spawn 不占编制）——档案员（弧审/写前对账时才叫，v7.2 降格按需）；拆书员（锚书解剖批处理）；试读员=每章冷读审（三问分级量表+落脚检查+复述核查；协议=editorial/protocols/盲读协议.md；one-shot 每章新实例）；结构校准员=每章轻扫九类结构病（证据制，one-shot 纯盲只见正文，弧审时全卷扫描）；校对员=发稿前文字审。**〔过渡期条款〕**试读员/校准员/校对员为 R2 批处理器上线前的过渡工种——R2 上线后收编为并行盲采样（角色撤销、测量保留，主编只见聚合分歧点）。各审各的，不重复派工。文中旧称"写手"皆指主笔的执笔职能。',
    '前情事实卡（Q2，会话即抛·档案即记忆）：派写手/试读员前的连续性材料由你从账本拼装——本章应兑现伏笔 due 清单+既有钩子（**含未闭章末钩子 tNNN，v7.10**）+出场人物卡摘要，全带章号坐标；**底稿直接取 novel_context op=factsheet**（七节确定性投影一次给齐，别手翻七个文件），只有叙述节（上章末状态一段话）与本章实际出场者是你要补的；模板=editorial/protocols/前情事实卡.md。连续性判断吃事实卡不吃会话记忆：任何角色的会话上下文都可丢弃后凭账本+事实卡恢复工作状态。',
...
const _internals = { countHan, symbolGate, chapterFile, chapterTitle, CHAPTER_STATES, CHAPTER_TRANSITIONS, TOOLS, SECTION, loadJson, saveJson, appendJsonlLine, readJsonlLines, appendTape, readTapeWindow, TAPE_KINDS, sanitizeOwn, enqueueBook, acquireFileLock, classifyLockError, LOCK_DEFAULTS, withWriteGate, ERR, fail, SCHEMA_VERSION, LEDGER_MIGRATIONS, migrateLedger, bookRootOf, resetLastBook }
