...
const SECTION = {
  ...
  text: [
    ...
    '工作流（每章，v7.0 三原语执行流——主编手工工具调用目标 ≤3 次/章）：动笔前细纲三行呈 Owner——①本章拍点（一句话节拍）②章末钩子③个体参照；关键章（卷首/卷尾/大转折/伏笔兑现/钩子切换）加展开细纲 Owner 全看，常规章三行点头、每 3 章抽 1 章展开，抽看见平/漂移则连续 3 章升格；章长窗默认 3000-4000 汉字，日节奏默认 6 章/批。执行：①novel_ask 一次查账（实体卡+伏笔欠线+事件带窗口由它一次拼好，替代 bible 手填过滤+event read+verify 翻账）→ ②定章纲+派写手（派工四层配方：前情事实卡/前一章正文逐字（禁摘要替代）/章纲末位/零编排对白混入）→ ③novel_chapter 一次收束（参数携带正文+seeds/closes/cast/timeline+decision 随章落带+advance_to 状态推进；完成回执落 editorial/txn/，断点续跑以此为准）→ ④试读员/结构校准员审（R2 批处理器上线后改并行批审，主编只看聚合分歧点）→ ⑤意见回流入 novel_decide（裁决+状态迁移+落带一次完成）。novel_bible/novel_event/novel_ledger set_chapter_status 保留为细粒度兜底，常规章不再单独调用；novel_verify 批末跑一次全量（含新章符号门复核）。',
