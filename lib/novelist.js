...
const TOOLS = [
  ...
  // 5. novel_verify：符号级自检（只读）
  {
    ...
    async execute(args, exec) {
      ...
      // 戏剧选择核对（v6.6，ADR-034 C-3）：书内出现过 choice_axis 才启用（存量书不误报）
      let choiceAdopted = false
      for (const vol of outline.volumes || []) { for (const c of vol.chapters || []) { if (c.choice_axis) choiceAdopted = true } }
      if (choiceAdopted) {
        for (const vol of outline.volumes || []) {
...
const SECTION = {
  ...
  order: 150,
  text: [
    'novelist-guide v6.6 · 机制细则唯一真源（novelist 插件同版注入；preset persona 只含决策原则与指针）。书工程 = book_dir（一部书一个目录；novel_* 工具一律显式传 book_dir 绝对路径）。身份与协作框架见 persona 段。v6 增量（2026-09-04）：冷读审法换三问量表、前情事实卡、盲角色派工纪律四条；v6.1：编辑部五角色→六角色（+结构校准员，one-shot 纯盲只见正文）；v6.2：P0 回改定稿前 A/B 冷读门、商业工位（书名简介/责编评分卡）；v6.3（修订轮）：校对员 one-shot+交付预算纪律（preset 侧）、编排熔断、写手落盘无题首行；v6.4（W-1 双臂实测后）：角色传输二分——策划/档案员=常驻同事（continuable 起+s2s session_id 唤醒），写手/试读/校准/校对=一次性前台 spawn（盲读与写入口安全不进同事线）；v6.5（V3 P1，2026-09-05）：事件带 novel_event（决策/判词/欠线/checkpoint 落带=编排层记忆与抗压缩记忆）＋仪器纪律预告（轻量门三查/检查点全量/证据引用制，细则随 P3 评分卡 v2）＋锚参字段与 selection_report gate＋版本戳三处对齐；v6.6（ADR-034，Owner 2026-09-13 裁）：戏剧工位（C-3——每卷开局 3 互斥走向呈 Owner 三选一，主编署名担责，互斥性失败=提案记零）＋结构主方法换向（C-2——单源节拍移植为准/统计降级守门尺/锚定目标=分布尾部）＋choice_axis 字段（verify 核启用卷卷首章必带）＋B9 传感器恢复（preset 侧撤阈值豁免）。',
    '工作流（卷级开局，ADR-034 v6.6）：戏剧选择呈报=主编出 3 互斥走向（各带：代价=放弃项逐条点名/承诺=挂卷内伏笔位/偏好自述=为何更疼）→ Owner 三选一 → 选中走向的代价清单以 choice_axis 落卷首章纲（novel_outline write）；结构骨架按单源节拍移植（C-2）取一本锚书连续真实章 beats 1:1 移植结构位（粒度=节拍级、可跨题材），统计参数只作守门尺防贫血。',
