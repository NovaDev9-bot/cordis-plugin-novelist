...
const SECTION = {
  ...
  order: 150,
  text: [
    'novelist-guide v7.9 · 机制细则唯一真源（novelist 插件同版注入；preset persona 只含决策原则与指针）。书工程 = book_dir（一部书一个目录；novel_* 工具一律显式传 book_dir 绝对路径）。身份与协作框架见 persona 段。**机制沿革（v6→v7 逐版增量：新增机制/口径修正/作废条款）见 ' + GUIDE_HISTORY_PATH + '——只在对某版细节有疑问时定向读，不必常驻上下文；本条以下各段即为当前生效口径。**',
