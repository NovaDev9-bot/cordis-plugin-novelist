...
async function acquireFileLock(fs, dir, opts) {
  ...
  for (;;) {
    try {
      ...
      const code = e && e.code
      // Windows delete-pending：同侪刚 rm、句柄未落时 open(...,'wx') 抛裸 EPERM/EACCES。
      // 判据不是错误码本身，而是"锁文件是否真在盘上"——在＝争用（走陈旧→重试→超时），
      // 不在＝真权限拒绝（目录只读一类），原样抛。差别在调用方拿到的药方：
      // 「等它写完再重试」和「你没权限」是两件事，混成一件就是把超时当成权限错。
      const contendedByCode = (code === 'EPERM' || code === 'EACCES') && !!(await stat(procPath).catch(() => null))
      const kind = classifyLockError(code, contendedByCode)
      if (kind === 'no-dir') {
...
/**
 * 锁获取失败的错误分类（纯函数，便于单测——真机只在 Windows delete-pending 窗口才撞得到）。
 * - `no-dir`：书目录还不存在（首次开书；写类工具据此先建目录再取锁）
 * - `contended`：锁被占——EEXIST，**或** EPERM/EACCES 且锁文件确实在盘上（delete-pending）
 * - `fatal`：其余原样抛（含 EPERM/EACCES 而锁文件不在＝真权限拒绝，不许伪装成超时）
 */
function classifyLockError(code, lockExists) {
  if (code === 'ENOENT') return 'no-dir'
  if (code === 'EEXIST') return 'contended'
  if ((code === 'EPERM' || code === 'EACCES') && lockExists) return 'contended'
  return 'fatal'
}
...
export {
  ...
  LOCK_DEFAULTS, HOST, WRITE_TOOLS, enqueueBook, acquireFileLock, classifyLockError, lockIsStale, isProcessAlive, withWriteGate,
}
