...
判据信度同理：宽松判据（within-1 一致率）下，一个只会恒定打 4 分的判官能拿满分信度。`dsh-native/plugin-novelist/instruments/instrument-aggregate.mjs` 强制报 exact agreement、ICC(2,1)、量程使用分布，sd=0 / n<5 的维度显式"不可测"，绝对分与成对判两把尺子打架时**阻断达标结论**。
