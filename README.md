...
**本仓不发原文，也不发任何带原文的卡——发能力，不发素材。** 两条理由：

- **实证**：要让模型换一副嗓子，最有效的做法是给它看目标语域的**原文**；把风格写成"句长中位 X、短句占比 Y"这类数字，被提示（prompting）与 LoRA 一致优于；更强的是，追加**可逐条核验**的约束反而把风格归属准确率拉低（Wang et al. 2025，400+ 位作者、4 万次生成）。机制是 extremal Goodhart——观测出的分布性质一旦变成逐句目标，模型就去凑比例、牺牲真语域。
- **法律**：**风格不受版权保护，载体受保护**。所以卡里写的是形态描述＋决策级禁令＋"该看哪几段"的坐标，原文由使用者从**自己合法持有的文本**生成。

```bash
# 1. 把锚书目录交给它（认两种布局：<锚书>/manuscript/chapter_NNN.md，或目录下任意 *.md/*.txt）
node instruments/build-author-card.mjs --book <你的锚书目录> --out 我的卡.md --json 实测.json
#    → 我的卡.md（给写手）：一段锚段 + 按**场景功能**挑的范例候选（对话戏配对话戏，不按主题相似度）
